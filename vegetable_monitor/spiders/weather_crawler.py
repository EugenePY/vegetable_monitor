...
class VegetableSpider_new(Spider):
    ...
    def __init__(self):
        ...
        today = datetime(date.today().year, date.today().month,
                         date.today().day) - timedelta(days=1)
...
class VegetableSpider(Spider):
    ...
    def parse(self, response):
        ...
        if data:
            item['date'] = date_trans(data[0][1][:9])
            item['data'] = data
            yield item
...
class SattelliteSpider(Spider):
    ...
    dat_list = [today]
    while dat_list[-1].day < date.today().day:
...
class WeatherSpider(Spider):
    ...
    def __init__(self):
        ...
        self.formdata = {'zipcode': zipcode,
                         'syr': zipcode.split('/')[0], 'smo': zipcode.split('/')[1],
...
if __name__ == "__main__":
    ...
    process.crawl(VegetableSpider_new)
    process.start()
