...
def check_health(func):
    if func._connect_status == 200:
        return func
    else:
        logger.debug("Function: %s failed, status: %i", (str(func),
...
def every(minutes=None, seconds=None):
    pass


class Crawler(object):
    """
    every crawler should inherent this object
    """
    def __init__(self):
        self.url = None
        self.name = None
...
class PTT(Crawler):
    def __init__(self, board, from_date=None):
        pass


def crawler_db(board, ):
